...
COMMENT = 'Nadam_Untouched_Reward'
...
FILE_WEIGHTS = os.path.join(tboard_drive,
                            ENV_NAME + COMMENT + KERNEL_INIT + '_weights' + ACTIVATION + str(NUM_HIDDEN_UNITS) + '.h5')
TENSORBOARD_LOG = os.path.join(tboard_drive,
                               ENV_NAME + COMMENT + KERNEL_INIT + str(NUM_HIDDEN_UNITS) + ACTIVATION + '_units_' + str(
                                   Timer()))
...
class Agent:
    def __init__(self, model):
        ...
        self.optimizer = tensorflow.keras.optimizers.Nadam(0.0004)#tf.optimizers.Nadam(0.0004)
    ...
    def action(self, obs):
        ...
        action = tf.one_hot(tf.cast(action, tf.int32), NUM_ACTIONS)
        action = tf.cast(action, tf.float32)
        return action, logits

    def PickActionFromLogits(self, logits):
        # return tf.random.categorical(tf.nn.softmax(logits).numpy().reshape(1,-1),1)
    ...
    def train(self, env, resume=True):
        if resume:
            try:
                ...
                print(e)
        writer = tf.compat.v1.summary.FileWriter(TENSORBOARD_LOG)#tf.summary.create_file_writer(TENSORBOARD_LOG)
        obs = env.reset()
        ...
        while True:
            ...
            # reward = 0
            # max_pos.append(n_obs[0])

            # reward =(n_obs[0]+0.3)*10 if n_obs[0] > -0.3 else 0 # remove offset
            ...
            if done:
                ...
                running_reward = 0 if running_reward == None else running_reward * 0.99 + self.memory.sum_rewards * 0.01
